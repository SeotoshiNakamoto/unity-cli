...
func primeCmd(project string, port int) error {
	...
	sb.WriteString("- UI 요소 탐색/조작: `ui tree`로 UIToolkit 트리 확인, `ui query \"type=Button\"`로 요소 검색, `ui click \"id=save-btn\"`로 클릭(전후 diff 자동 반환). `ui snapshot`은 JSON+스크린샷 동시 캡처.\n")
	sb.WriteString("- 비동기 화면 전환 감지: `ui events`로 UIDocument 추가/제거 이벤트 확인. 로딩 화면 완료, 새 UI 등장 등을 감지.\n")
	sb.WriteString("\n## async 실행\n")
